...
type PanelDailyState = { sets: Record<string, boolean>; foods: Record<string, boolean>; water: number; supplements: Record<string, boolean>; expanded: Record<string, boolean>; timers: Record<string, PanelTimer>; celebrations: Record<string, boolean> };
const EMPTY_DAILY_STATE: PanelDailyState = { sets: {}, foods: {}, water: 0, supplements: {}, expanded: {}, timers: {}, celebrations: {} };
...
function readPanelState(key: string): PanelDailyState {
  try { const saved = JSON.parse(localStorage.getItem(key) ?? "{}") as Partial<PanelDailyState>; return { sets: saved.sets ?? {}, foods: saved.foods ?? {}, water: Math.max(0, Math.min(10, Number(saved.water) || 0)), supplements: saved.supplements ?? {}, expanded: saved.expanded ?? {}, timers: saved.timers ?? {}, celebrations: saved.celebrations ?? {} }; }
...
function Workout({ data, user, go, reload }: { data: Json; user: User; go: (view: View) => void; reload: () => Promise<void> }) {
  ...
  const saveRecord=async(exercise:Json,key:string)=>{const entered=Number(weights[key]),name=textOf(exercise,["name","exercise_name","title"],"حرکت تمرینی"),previous=Math.max(0,...records.filter(item=>textOf(item,["exercise_name","name"])===name).map(item=>numberOf(item,["value","weight"])));if(Number.isFinite(entered)&&entered>previous){await api("/me/records/",{method:"POST",body:JSON.stringify({exercise_name:name,value:entered,unit:"کیلوگرم",achieved_on:today()})});setRecordSuccess({name,value:entered})}};
  const toggleSet=async(exercise:Json,index:number,exerciseIndex:number)=>{if(expired)return;const key=exerciseKey(exercise,exerciseIndex),setKey=`${key}:${index}`,current=setDone(exercise,index,exerciseIndex),wasComplete=Array.from({length:setCount(exercise)},(_,i)=>setDone(exercise,i,exerciseIndex)).every(Boolean),materialized=Object.fromEntries(Array.from({length:setCount(exercise)},(_,i)=>[`${key}:${i}`,setDone(exercise,i,exerciseIndex)])),nextSets={...daily.sets,...materialized,[setKey]:!current},complete=Array.from({length:setCount(exercise)},(_,i)=>Boolean(nextSets[`${key}:${i}`])).every(Boolean),superset=pick(exercise,"superset_with","superset_id","paired_exercise"),total=Math.max(10,numberOf(exercise,["rest_seconds","rest_sec","restSec","rest"],60));setActionError("");setDaily(value=>{const timers={...value.timers};if(!current&&!superset){for(const timerKey of Object.keys(timers))timers[timerKey]={...timers[timerKey],running:false};timers[key]={remaining:total,total,running:true,done:false}}return{...value,sets:{...value.sets,...materialized,[setKey]:!current},timers}});if(complete===wasComplete)return;setPendingExercise(key);try{await syncExercise(exercise,complete);const exerciseId=String(pick(exercise,"id","exercise_id"));setServerDone(value=>({...value,[exerciseId]:complete}));if(complete)await saveRecord(exercise,key)}catch{setDaily(value=>({...value,sets:{...value.sets,...Object.fromEntries(Array.from({length:setCount(exercise)},(_,i)=>[`${key}:${i}`,Boolean(materialized[`${key}:${i}`])]))}}));setActionError("ثبت وضعیت حرکت انجام نشد؛ وضعیت قبلی بازیابی شد.")}finally{setPendingExercise(null)}};
  ...
  return <div className={styles.dailyPanel}>
    ...
    <section className={styles.workoutSummary}><div><small>برنامه تمرینی امروز</small><h2>{textOf(plan,["title","name"],"برنامه تمرینی")}</h2><p><b>{textOf(plan,["coach_name","trainer_name"],"مربی شما")}</b><span>هفته {fa(pick(plan,"current_week","week")??1)}</span><span>{fa(checkedSets)} / {fa(totalSets)} ست</span></p></div><Ring value={progress} label={<><b>{fa(progress)}٪</b><small>پیشرفت</small></>}/></section>
    ...
    {restDay?<div className={styles.dailyRestDay}><span>🌿</span><b>امروز روز استراحت است</b><p>ریکاوری، آب کافی و خواب باکیفیت را جدی بگیر.</p></div>:<section className={styles.dailyList}>{exercises.map((exercise,exerciseIndex)=>{const key=exerciseKey(exercise,exerciseIndex),sets=setCount(exercise),done=Array.from({length:sets},(_,index)=>setDone(exercise,index,exerciseIndex)).every(Boolean),open=daily.expanded[key]??exerciseIndex===0,name=textOf(exercise,["name","exercise_name","title"],"حرکت تمرینی"),note=textOf(exercise,["note","notes","coach_note"]),alternative=textOf(exercise,["alternative","alternative_name"]),muscle=textOf(exercise,["muscle","muscle_group","target_muscle"]),warmup=textOf(exercise,["warmup","warm_up","warmup_note"]),previous=Math.max(0,...records.filter(item=>textOf(item,["exercise_name","name"])===name).map(item=>numberOf(item,["value","weight"]))),timer=daily.timers[key],superset=textOf(exercise,["superset_name","superset_with_name","paired_exercise_name"],pick(exercise,"superset_with","superset_id")?"حرکت بعدی":"");return <article className={`${styles.dailyAccordion} ${done?styles.dailyAccordionDone:""}`} key={key}><button type="button" className={styles.dailyAccordionHead} aria-expanded={open} onClick={event=>{event.preventDefault();setDaily(current=>({...current,expanded:{...current.expanded,[key]:!open}}))}}><span>{done?<Icon name="check" size={18}/>:fa(exerciseIndex+1)}</span><p><b>{name}</b><small>{fa(sets)} ست × {fa(pick(exercise,"reps","repetitions")??"-")} تکرار {superset?"· سوپرست":""}</small></p><em className={styles.accordionChevron}>⌄</em></button>{open?<div className={styles.dailyAccordionBody}>{muscle||warmup?<div className={styles.dailyTags}>{muscle?<span>{muscle}</span>:null}{warmup?<span>گرم‌کردن: {warmup}</span>:null}</div>:null}{note?<p className={styles.coachNote}><Icon name="coach" size={15}/>{note}</p>:null}{superset?<p className={styles.supersetLink}>بدون استراحت با <b>{superset}</b></p>:null}<div className={styles.setList}>{Array.from({length:sets},(_,setIndex)=>{const checked=setDone(exercise,setIndex,exerciseIndex),detail=setDetail(exercise,setIndex),reps=pick(detail,"reps","repetitions")??pick(exercise,"reps","repetitions")??"-",weight=pick(detail,"weight","kg","load")??pick(exercise,"weight","kg","load")??weights[key]??"-",type=setType(exercise,setIndex);return <label className={checked?styles.setDone:""} key={setIndex}><input type="checkbox" checked={checked} disabled={pendingExercise===key||expired} onChange={event=>{event.preventDefault();void toggleSet(exercise,setIndex,exerciseIndex)}}/><b>{fa(setIndex+1)}</b><span><em className={type==="گرم‌کردن"?styles.setTypeWarm:type==="اصلی"?styles.setTypeWork:styles.setTypeDanger}>{type}</em></span><small><b>{fa(reps)}</b> تکرار</small><small><b>{fa(weight)}</b> کیلوگرم</small></label>})}</div>{!activeTimer&&timer?<div className={`${styles.exerciseTimer} ${timer.remaining<=10&&timer.remaining>0?styles.timerWarning:""} ${timer.done?styles.timerDone:""}`}><p><Icon name="clock"/><b>{timer.done?"استراحت تمام شد":`${fa(Math.floor(timer.remaining/60))}:${String(timer.remaining%60).padStart(2,"0")}`}</b><small>استراحت {name}</small></p><div><button onClick={()=>changeTimer(key,"toggle")}>{timer.running?"توقف":"ادامه"}</button><button onClick={()=>changeTimer(key,"reset")}>از نو</button><button onClick={()=>changeTimer(key,"skip")}>رد کردن</button></div></div>:null}<div className={styles.dailyExerciseTools}><label>وزنه امروز<input inputMode="decimal" value={weights[key]??""} disabled={!attendance?.is_active||done} onChange={event=>setWeights(current=>({...current,[key]:event.target.value}))} placeholder="کیلوگرم"/></label>{previous>0?<small>رکورد قبلی: {fa(previous)} کیلوگرم</small>:null}{alternative?<small>جایگزین: {alternative}</small>:null}{pick(exercise,"media_url","video_url","media")?<button onClick={()=>setVideo(exercise)}><Icon name="play" size={14}/> فیلم آموزشی</button>:null}</div></div>:null}</article>})}</section>}
...
function Nutrition({ data, user, go, reload }: { data: Json; user: User; go: (view: View) => void; reload: () => Promise<void> }) {
  ...
  const completedMeals=meals.filter(mealDone).length,allDone=meals.length>0&&completedMeals===meals.length;
  ...
  return <div className={styles.dailyPanel}>
    <DailyHeader kind="nutrition" user={user} go={go} title={textOf(plan,["title","name"],"برنامه غذایی روزانه")} eyebrow={`تغذیه ${editable?"امروز":"روز انتخاب‌شده"} · ${date(selectedDate)}`} detail={`${fa(completedMeals)} از ${fa(meals.length)} وعده تکمیل شده`} progress={calorieProgress} streak={numberOf(data,["streak","nutrition_streak","consecutive_days"])} ringLabel={<><b>{targetCalories?fa(consumed):`${fa(calorieProgress)}٪`}</b><small>{targetCalories?`از ${fa(targetCalories)} کالری`:"کالری ثبت‌شده"}</small></>}/>
